...
do
  ...
  sed 's/ \+/,/g' /media/sf_AIDD/raw_data/snpEff/snpEff"$run".genes.txt > /media/sf_AIDD/raw_data/snpEff/"$run""$condition"genes.csv
done < $INPUT
IFS=$OLDIFS
for i in gene transcript ; do
for j in high moderate ; do
cp /media/sf_AIDD/raw_data/snpEFF/*mockgenes.csv /media/sf_AIDD/Results/variant_calling/haplotype/"$i"/"$j"_impact/mock/
cp /media/sf_AIDD/raw_data/snpEFF/*zikagenes.csv /media/sf_AIDD/Results/variant_calling/haplotype/"$i"/"$j"_impact/zika/
done
done
} 
...
```
##snpEff tables2
This is the next step for creating high and moderate impact tables
```{r}
...
path <- "/media/sf_AIDD/Results/variant_calling/haplotype/gene/high_impact/zika/"
DF <- multmerge(path)
write.csv(DF, "/media/sf_AIDD/Results/variant_calling/haplotype/gene/high_impact/zikahighFinal.csv", row.names=FALSE)
multmerge = function(path){
  filenames=list.files(path=path, full.names=TRUE)
  rbindlist(lapply(filenames, fread))
}
...
path <- "/media/sf_AIDD/Results/variant_calling/haplotype/gene/moderate_impact/zika/"
DF <- multmerge(path)
write.csv(DF, "/media/sf_AIDD/Results/variant_calling/haplotype/gene/moderate_impact/zikamoderateFinal.csv", row.names=FALSE)
multmerge = function(path){
  filenames=list.files(path=path, full.names=TRUE)
  rbindlist(lapply(filenames, fread))
}
path <- "/media/sf_AIDD/Results/variant_calling/haplotype/gene/moderate_impact/mock/"
DF <- multmerge(path)
write.csv(DF, "/media/sf_AIDD/Results/variant_calling/haplotype/gene/moderate_impact/mockmoderateFinal.csv", row.names=FALSE)

multmerge = function(path){
  filenames=list.files(path=path, full.names=TRUE)
  rbindlist(lapply(filenames, fread))
}
path <- "/media/sf_AIDD/Results/variant_calling/haplotype/gene/high_impact/"
DF <- multmerge(path)
write.csv(DF, "/media/sf_AIDD/Results/variant_calling/haplotype/gene/high_impact/mockmoderateFinal.csv", row.names=FALSE)
```

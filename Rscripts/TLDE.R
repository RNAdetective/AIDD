...
nrow(dds)
jpeg("rlogandvariance_trans.jpeg")
lambda <- 10^seq(from = -1, to = 2, length = 1000)
cts <- matrix(rpois(1000*100, lambda), ncol = 100)
meanSdPlot(cts, ranks = FALSE)
dev.off()
jpeg("logtranscounts_trans.jpeg")
...
jpeg("transcounts2sam_trans.jpeg")
...
jpeg("MAplot_trans.jpeg")
plotMA(res, ylim = c(-5, 5))
dev.off()
jpeg("Histopvalue_trans")
...
tablebargraph4 <- aggregate(tablebargraph3[, 3:13], list(tablebargraph3$sample), mean)
...
tablebargraph2
tablebargraph3 <- merge(Data, tablebargraph2, by="t_name")
tablebargraph4 <- aggregate(tablebargraph3[, 3:4], list(tablebargraph3$sample), mean)
tablebargraph5 <- t(tablebargraph4)
tablebargraph5 <- tablebargraph5[-c(1),]
write.csv(tablebargraph5, "5.csv")
write.csv(tablebargraph5, "6.csv")
...
tablebargraph5 <- merge(tablebargraph5, names1, by="t_name")
tablebargraph6 <- merge(tablebargraph6, names1, by="t_name")
final <- rbind(tablebargraph5, tablebargraph6)
tiff("ADAR1isoformimpbar1.tiff")
ggplot(final, aes(x=isoform, y=V1, fill=condition)) +
    geom_bar(stat="identity", position=position_dodge(), colour="black")
dev.off()

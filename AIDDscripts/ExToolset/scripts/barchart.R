...
colnames(data_in)[5] <- "condition_name_name"
colnames(data_in)[7] <- "freq"
tiff(file_out, units="in", width=10, height=10, res=600) #names the chart file
q <- ggplot(data_in, aes(x=name, y=freq, fill=condition_name_name)) + geom_bar(stat="identity", color="black", position=position_dodge()) + theme_minimal() + theme(legend.position="bottom") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
condition_name_name <- paste0(args[7])
pheno_in <- read.csv(pheno)
data_in <- read.csv(file_in);
  if ( condition_name_name == "samp_name" ) {
  colnames(data_in)[1] <- "samp_name"
  datamerge <- merge(pheno_in, data_in, by="samp_name")
  }
  if ( condition_name_name == "condition_name_name" ) {
  colnames(data_in)[1] <- "condition_name_name"
  datamerge <- merge(pheno_in, data_in, by="condition_name_name")
  }
cdata <- ddply(datamerge, c("condition_name_name"), summarise, N = length(condition_name_name), mean=round(mean(condition_name_name),2), sd=round(sd(condition_name_name), 2))
cdata$substitution <- rep("condition_name_name",nrow(cdata)) # make new column 
write.csv(cdata, sum_file, row.names=FALSE, quote=FALSE)
#tiff(file_out, units="in", width=10, height=10, res=600) #names the chart file
#q <- ggplot(cdata, aes(x=condition_name_name, y=mean, fill=condition_name_name)) + geom_bar(stat="identity", color="black", position=position_dodge()) + theme_minimal() + theme(legend.position="bottom") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
#p <- q + theme(axis.text.x = element_text(angle = 90, hjust = 1))
#r <- p + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2, position=position_dodge(.9)) 
#z <- r + labs(title="condition_name_name expression for condition_name_name", x="condition_name_name", y = "Expression(TPM)")
#print(z)
#garbage <- dev.off()
}
...
tiff(file_out, units="in", width=10, height=10, res=600) #names the chart file
q <- ggplot(cdata, aes(x=substitution, y=mean, fill=condition_name_name)) + geom_bar(stat="identity", color="black", position=position_dodge()) + theme_minimal() + theme(legend.position="bottom") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p <- q + theme(axis.text.x = element_text(angle = 90, hjust = 1))
r <- p + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2, position=position_dodge(.9)) 
...
if ( bartype == "ANOVA" ) {
  ...
  cdata <- ddply(data, c("condition_name_name"), summarise, N = length(freq_name), mean=round(mean(freq_name),5), sd=round(sd(freq_name), 5))
  ...
  p <- q + theme(axis.text.x = element_text(angle = 90, hjust = 1))
  r <- p + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2, position=position_dodge(.9)) 
  z <- r + labs(title="condition_name_name expression for freq_name", x="freq_name", y = "Expression(TPM)")
  print(z)
  garbage <- dev.off()
  tiff(file_out2, units="in", width=10, height=10, res=600)
  p <- ggboxplot(data, x= "condition_name_name", y = "freq_name", color = "condition_name_name")
  print(p)
  garbage <- dev.off()
res.aov <- anova(lm(freq_name~condition_name_name, data=data))

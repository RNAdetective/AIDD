...
ggplot(pcaData, aes(x = PC1, y = PC2, color = set_design, group = set_design, label=rownames(pcaData))) + geom_point(size = 3) + xlab(paste0("PC1: ", percentVar[1], "% variance")) + ylab(paste0("PC2: ", percentVar[2], "% variance")) + coord_fixed() + geom_text_repel(aes(label=rownames(pcaData)),position = position_dodge(width=0.9),size=8) + theme(legend.position="bottom") + theme(text = element_text(size = 18))
invisible(dev.off()) #save tiff
...
pheatmap(mat, annotation_col = anno, annotation_legend=FALSE, fontsize_row=8)
invisible(dev.off()) #save tiff
...
volc + geom_text_repel(data=head(res_tableOE_ordered, 20), aes(label = levellabels), size = 8) + theme(text = element_text(size = 16))
invisible(dev.off())

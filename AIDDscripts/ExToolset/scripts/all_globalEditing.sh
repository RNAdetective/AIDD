...
for batch in B9 batch01_07 batch08 batch09 batch10 batch11 batch12 batch13 batch14 batch15 batch16 batch17 batch18 batch19 batch20 batch21 batch22 batch23 batch24 batch25 batch26 batch27 ; 
do 
  dir=/media/sf_AIDD/MDD"$batch"/Results/guttman/stackdepth ;
  fdir=/media/sf_AIDD/all_stackdepth ;
  for files in "$dir"/* ;
  do 
    dir_name=$(echo "$files" | sed 's/\//./g' | cut -f 8 -d '.')
  ...
  do     
    ...
    do  
      dir=/media/sf_AIDD/MDD"$batch"/Results/variant_calling ;     
      fdir=/media/sf_AIDD/all_globalEditing;
      new_dir="$fdir"/"$type";
      create_dir;

...
single_col() {
header=$(head -n 1 "$dir_path"/"$filter".csv)
cat "$dir_path"/"$filter".csv | awk -F "," ' { if (($'$col_num' == "'$infection'")) { print } }' | sed 's/ /,/g' | sed '1i '$header'' > "$new_dir"/"$infection".csv
}
double_col(){
header=$(head -n 1 "$dir_path"/"$filter".csv)
cat "$dir_path"/"$filter".csv | awk -F "," ' { if (($'$col_num1' == "'$infection'") && ($'$col_num2' == "'$Timepoint'")) { print } }' | sed 's/ /,/g' | sed '1i '$header'' > "$new_dir"/"$infection"_"$Timepoint".csv
}
triple_col() {
header=$(head -n 1 "$dir_path"/"$filter".csv)
cat "$dir_path"/"$filter".csv | awk -F "," ' { if (($'$col_num1' == "'$infection'") && ($'$col_num2' == "'$Timepoint'") && ($'$col_num3' == "'$CellLine'")) { print } }' | sed 's/ /,/g' | sed '1i '$header'' > "$new_dir"/"$infection"_"$Timepoint"_"$CellLine".csv
}
temp_file() {
...
for mainfolder in COVID19single COVID19Paired ;
do
  for filter in guttman0percent guttman1percent guttman5percent ;
  do
    dir_path=/media/user/ExtraSpace3/"$mainfolder"/Results/"$filter"
    ExToolset=/home/user/AIDD/AIDD/ExToolset/scripts
    new_dir="$dir_path"/GuttmanAllinfection
    create_dir
    for infection in mock SARS_CoV1 SARS_CoV2 ;
    do
      for Timepoint in 04h 12h 24h ;
      do
    ...
    temp_file
    summaryfile="$dir_path"/"$filter".csv
    phenofile="$dir_path"/"$filter".csv 
    level_name=pid 
    file_in="$mergefile"

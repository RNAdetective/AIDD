...
bam_dir="$out_dir"/raw_data/bamfiles #defining where bam files are located
...
do
  ...
  file_ext=$(echo "$files" | sed 's/\//./g' | cut -f 8 -d '.')
  ...
  vcf_file="$vcf_dir"/"$file_name"filtered_snps_finalAll.vcf #name of vcf_file based on identifier (run #) bam file and vcf file have to have same identifier
  echo "$vcf_file"
